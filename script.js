...
const apiKey = '3680680682b011dfd75760df5df3ef55';
...
async function updateForecastsInfo(city) {
  ...
  const timeTaken = '12:00:00'; // Biramo prognozu u 12:00 za svaki dan
  const todayDate = new Date().toISOString().split('T')[0];

  forecastItemsContainer.innerHTML = ''; // Očisti prethodne prognoze
  let addedDays = 0; // Broj dana koje dodajemo u prognozu

  forecastsData.list.forEach((forecastsWeather) => {
    if (forecastsWeather.dt_txt.includes(timeTaken)) {
      if (addedDays < 7) { // Dodajemo do 7 dana
...
function updateForecastsItems(weatherData) {
  ...
  const dateOption = {
    weekday: 'short', // Dodajemo dan u sedmici (npr. Fri)
    day: '2-digit',
    month: 'short',
  };
